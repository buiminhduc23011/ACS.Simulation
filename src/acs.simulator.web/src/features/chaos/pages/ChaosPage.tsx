...
export const ChaosPage: React.FC = () => {
  ...
  const [chaosSettings, setChaosSettings] = useState<IChaosSettings>({
  ...
  const injectTemplate = async (templateName: string) => {
    if (!selectedAgv) { message.warning('Select an AGV first'); return; }
    setLoading(true);
    try {
      // Backend InjectTemplateRequest binds to `TemplateName` (not `templateId`).
      await apiClient.post(ENDPOINTS.control.injectTemplate(selectedAgv), { templateName });
  ...
  const applyChaos = async () => {
    ...
    setLoading(true);
    try {
      // Backend ChaosSettingsRequest requires LatencyEnabled and PacketLossEnabled booleans;
      // without them the chaos service ignores the numeric values entirely.
      // Derive the flags from whether slider values are non-zero.
      const payload: IChaosSettings = {
        ...chaosSettings,
        latencyEnabled: chaosSettings.latencyMinMs > 0 || chaosSettings.latencyMaxMs > 0,
        packetLossEnabled: chaosSettings.packetLossPercent > 0,
      };
      await apiClient.post(ENDPOINTS.control.chaos(selectedAgv), payload);
  ...
  const disconnect = async () => {
    ...
    await apiClient.post(ENDPOINTS.control.disconnect(selectedAgv), { durationMs: 30000 });
          ...
          <Card title="Target AGV">
            ...
            <Form layout="vertical">
              ...
              <Space>
                ...
                <Button onClick={() => {
                  const resetSettings: IChaosSettings = { latencyEnabled: false, latencyMinMs: 0, latencyMaxMs: 0, packetLossEnabled: false, packetLossPercent: 0 };
                  setChaosSettings(resetSettings);
                  if (selectedAgv) apiClient.post(ENDPOINTS.control.chaos(selectedAgv), resetSettings);
                }}>Reset</Button>
