...
public class SimulatorService
{
    ...
    public async Task<AgvSummaryDto?> CreateAgvAsync(CreateAgvRequest req)
    {
        ...
        try
        {
            ...
            _fleet[id] = agv;
            _meta[id] = new AgvMeta { Id = id, Manufacturer = req.Manufacturer, Speed = 1.0 };
    ...
    public async Task<AgvSummaryDto?> UpdateAgvAsync(string id, UpdateAgvRequest req)
    {
        ...
        try
        {
            ...
            _fleet[newId] = newAgv;
            _meta[newId] = new AgvMeta { Id = newId, Manufacturer = req.Manufacturer, Speed = 1.0 };
    ...
    {
        if (!_fleet.TryGetValue(id, out var agv)) return false;
        await agv.AddErrorAsync(req.ErrorType, req.ErrorLevel, req.Description);
    ...
    public async Task RestoreFleetAsync()
    {
        ...
        foreach (var saved in savedAgvs)
        {
            try
            {
                ...
                // Auto-start if it was connected before
                if (saved.AutoStart)
                {
